...
class LLMAdapter:
    ...
    def call(
        ...
        if model_lower.startswith("claude") or model_lower.startswith("anthropic/"):
            ...
            return self._call_dashscope(
                actual_model, system, user, temperature, max_tokens
            )
        else:
            # Try anthropic as default
            return self._call_anthropic(
                model, system, user, temperature, max_tokens, stop_sequences
            )
    ...
    def call_with_fallback(
        ...
        for i, m in enumerate(models_to_try):
            try:
                return self.call(
                    m, system, user, temperature, max_tokens, stop_sequences
                )
                ...
                print(
                    f"[Fallback] {m} failed: {e}. Trying {models_to_try[i + 1]}...",
                    file=sys.stderr,
                )
    ...
    def _call_anthropic(
        self, model, system, user, temperature, max_tokens, stop_sequences
    ):
        try:
            import anthropic
        except ImportError:
