...
def detect_framework(cfg: dict) -> str:
    ...
    if (
        "llm_config" in keys
        or "is_termination_msg" in keys
        or "assistant_agent" in keys
    ):
        return "autogen"
...
def lint_config(path_or_cfg) -> List[LintResult]:
    ...
    # ── L003: maxSteps = 0 ──
    max_steps = react_cfg.get(
        "maxSteps", cfg.get("max_iter", cfg.get("max_iterations", 10))
    )
    if agent_type == "react" and max_steps == 0:
        results.append(
            LintResult(
                "ERROR",
                "L003",
                "react.maxSteps = 0, agent won't execute",
                "Y_0(g) = _|_",
                framework,
            )
        )
    ...
    if temp > 1.5:
        results.append(
            LintResult(
                "WARN",
                "L007",
                f"temperature={temp} > 1.5, output may be unstable",
                "High entropy in stochastic parameter",
                framework,
            )
        )
    ...
    if memory_cfg.get("enabled") and memory_cfg.get("ttl", 3600) == 0:
        results.append(
            LintResult(
                "WARN",
                "L008",
                "memory.ttl=0, bindings never expire",
                "Gamma' bindings never reclaimed",
                framework,
            )
        )
    ...
    if memory_cfg.get("enabled") and memory_cfg.get("size", 20) == 0:
        results.append(
            LintResult(
                "WARN",
                "L009",
                "memory.size=0, memory has no effect",
                "Gamma' = Gamma union empty = Gamma",
                framework,
            )
        )
    ...
    if agent_type == "react":
        ...
        local_non_term = [
            t
            for t in mcp_cfg.get("localTools", [])
            ...
            if framework not in ("crewai", "langchain", "autogen"):
                results.append(
                    LintResult(
                        ...
                        "Pure reasoning loop, agent can only think",
                        framework,
                    )
                )
    ...
    if isinstance(retry, (int, float)) and retry > 5:
        results.append(
            LintResult(
                "WARN",
                "L012",
                f"guard.retry={retry} > 5, excessive retries",
                "Too many retries on dependent type check",
                framework,
            )
        )
    ...
    if agent_type == "router" and router_cfg and "default" not in router_cfg:
        results.append(
            LintResult(
                ...
                "CASE not exhaustive, unclassified input will error",
                framework,
            )
        )
    ...
    if memory_cfg.get("enabled"):
        ...
        results.append(
            LintResult(
                ...
                f"Gamma' = Gamma union store({strategy})",
                framework,
            )
        )
    ...
    if rag_cfg.get("enabled"):
        results.append(
            LintResult(
                "INFO",
                "L016",
                "RAG enabled, agent can access knowledge base",
        ...
        if isinstance(val, (int, float)) and val > 100:
            results.append(
                LintResult(
                    ...
                    f"Y_{{{int(val)}}} ≈ Y (unbounded) — high cost risk",
                    framework,
                )
            )
            break
                ...
                if isinstance(target, str) and not target.strip():
                    results.append(
                        LintResult(
                            ...
                            f"CASE branch {label} -> _|_ (undefined)",
                            framework,
                        )
                    )
    ...
    if sandbox_cfg:
        if (
            sandbox_cfg.get("network", False)
            and sandbox_cfg.get("filesystem", "full") == "full"
        ):
            results.append(
                LintResult(
                    "WARN",
