...
class TestChunkSplitter:
    ...
    def test_tool_function(self):
        ...
        result = chunk_split(
            {
                "text": "Hello world with some content.\n\nSecond paragraph with more.\n\nThird paragraph here.",
                "strategy": "paragraph",
                "chunk_size": 40,
            }
        )
...
class TestDocGen:
    def test_html_generation(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            result = doc_generate(
                {"source": md_path, "format": "html", "output": html_path}
            )
            assert "OK" in result
...
class TestKBManager:
    def test_create_and_list(self):
        ...
        try:
            result = kb_manage(
                {"action": "create", "name": name, "description": "Test KB"}
            )
            assert "OK" in result
    ...
    def test_add_and_search(self):
        ...
        try:
            ...
            try:
                ...
                assert "OK" in result
                result = kb_manage(
                    {"action": "search", "name": name, "query": "Lambda calculus"}
                )
