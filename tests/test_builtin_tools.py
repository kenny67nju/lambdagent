...
class TestEditFile:
    ...
    def test_edit_not_found(self):
        ...
        try:
            result = edit_file(
                {"file_path": path, "old_string": "xyz", "new_string": "123"}
            )
            assert "ERROR" in result
    ...
    def test_search_files_only(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            result = search_content(
                {"pattern": "match here", "path": d, "output_mode": "files_only"}
            )
            assert "a.py" in result
...
class TestE2EBuiltinTools:
    ...
    def test_write_list_search(self):
        ...
        from lambdagent.builtin_tools.file_tools import (
            write_file,
            list_files,
            search_content,
        )
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            write_file(
                {"file_path": path, "content": "def main():\n    print('hello')\n"}
            )
            # List
    ...
    def test_bash_git_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            run_bash(
                {"command": "git config user.email 'test@test.com'", "working_dir": d}
            )
            ...
            try:
                status = git_status("")
                assert (
                    "nothing to commit" in status.lower()
                    or status.strip() == "[OK] (no output)"
                )
            finally:
...
class TestCodeSearch:
    def test_search_function(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            result = code_search(
                {"query": "hello_world", "language": "python", "path": d}
            )
...
class TestTaskManager:
    ...
    def test_tool_functions(self):
        from lambdagent.builtin_tools.task_manager import (
            task_create,
            task_list,
            task_update,
            _default_manager,
